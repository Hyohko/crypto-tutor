...
void test_all_nist_sign_verify_vectors(void) {
    ...
    mpz_t plaintext, signature, expected_signature, should_be_valid_modulus,
        output;

    rsa_init(&public_key);
    rsa_init(&private_key);
    mpz_inits(
        plaintext, signature, expected_signature, should_be_valid_modulus,
        output, NULL
    );
    ...
    for (size_t i = 0; i < NUM_NIST_TESTS; ++i) {
        ...
        sprintf(
            msg_buffer, "NIST Sign/Verify Test Vector %zu", i + 1
        );  // Use %zu for size_t

        // Set keys - the text exponent is always 0x10001, the default exponent
        int result_pub = rsa_set_pubkey(
            &public_key, RSA_TEST_MODULI[i], strlen(RSA_TEST_MODULI[i]),
            RSA_DEFAULT_PUBLIC_EXPONENT, RSA_BASE_HEX
        );
        char pub_key_msg[8192];
        sprintf(
            pub_key_msg, "%s: rsa_set_pubkey failed for N=%s, E=%d", msg_buffer,
            RSA_TEST_MODULI[i], RSA_DEFAULT_PUBLIC_EXPONENT
        );
        TEST_ASSERT_EQUAL_INT_MESSAGE(RSA_SUCCESS, result_pub, pub_key_msg);

        int result_priv = rsa_set_privkey(
        ...
        sprintf(
            priv_key_msg, "%s: rsa_set_privkey failed for P=%s, Q=%s, E=%d",
            msg_buffer, RSA_TEST_PRIMES_P[i], RSA_TEST_PRIMES_Q[i],
        ...
        sprintf(
            mod_mismatch_pub_msg, "%s: Public key modulus N does not match P*Q",
            msg_buffer
        );
        TEST_ASSERT_EQUAL_GMP_MPZ(
            should_be_valid_modulus, public_key.n, mod_mismatch_pub_msg
        );

        char mod_mismatch_priv_msg[8192];
        sprintf(
            mod_mismatch_priv_msg,
            "%s: Private key modulus N does not match P*Q", msg_buffer
        );
        TEST_ASSERT_EQUAL_GMP_MPZ(
            should_be_valid_modulus, private_key.n, mod_mismatch_priv_msg
        );
        ...
        // Sign
        rsa_error_t ret_sign =
            rsa_mpz_private(&private_key, signature, plaintext);

        if (expect_fail) {
            char sign_fail_msg[8192];
            sprintf(
                sign_fail_msg,
                "%s: Signing was expected to fail but succeeded.", msg_buffer
            );
            ...
            sprintf(
                sign_succ_msg, "%s: Signing failed unexpectedly.", msg_buffer
            );
            ...
            sprintf(
                sig_match_msg,
                "%s: Signature does not match expected signature.", msg_buffer
            );
            TEST_ASSERT_EQUAL_GMP_MPZ(
                expected_signature, signature, sig_match_msg
            );
        }
        ...
        sprintf(
            verify_succ_msg, "%s: Verification failed unexpectedly.", msg_buffer
        );
        ...
        sprintf(
            verify_match_msg,
            "%s: Verification output does not match plaintext.", msg_buffer
        );
