...
/*void setUp(void) {
    ...
    // rsa_init(&test_key_privexp); // Example if a global key were used across multiple tests in this file
}
...
void test_all_nist_private_exponent_vectors(void) {
    ...
    for (size_t i = 0; i < NUM_NIST_TESTS; ++i) {
        ...
        // Note: rsa_set_privkey is expected to take hex strings for P and Q, and E.
        // Assuming E is a global hex string constant like "10001" or similar.
        // The length argument for rsa_set_privkey for E might need adjustment if E is not a fixed length.
        // For this example, assuming RSA_TEST_PUBLIC_EXPONENT_E is a simple hex string like "10001" (which is base 16).
        ...
        if (result == RSA_SUCCESS) {
            ...
            TEST_ASSERT_EQUAL_INT_MESSAGE(RSA_SUCCESS, result, "rsa_compute_private_exponent failed");

            if (result == RSA_SUCCESS) {
                TEST_ASSERT_EQUAL_GMP_MPZ(expected_d, private_key.d, "Private exponent mismatch");
            }
        }

        mpz_clear(expected_d);
        rsa_clear(&private_key); // Clear mpz_t members within the key for the next iteration
        rsa_init(&private_key);  // Re-initialize for the next iteration to ensure a clean state (especially if rsa_clear doesn't fully reset)
    }
