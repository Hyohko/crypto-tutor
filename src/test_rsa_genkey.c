...
void test_genkey_1024_multiple_times(void) {
    for (int i = 0; i < 10; ++i) {
        ...
        rsa_error_t ret = rsa_genkey(&newkey, 1024, RSA_DEFAULT_PUBLIC_EXPONENT, strlen(RSA_DEFAULT_PUBLIC_EXPONENT));
...
void test_genkey_invalid_size_too_small(void) {
    ...
    rsa_error_t ret = rsa_genkey(&badkey, RSA_MIN_KEY_BITLEN - 24, RSA_DEFAULT_PUBLIC_EXPONENT, strlen(RSA_DEFAULT_PUBLIC_EXPONENT));
...
void test_genkey_invalid_size_too_large(void) {
    ...
    rsa_error_t ret = rsa_genkey(&badkey, RSA_MAX_KEY_BITLEN + 7232, RSA_DEFAULT_PUBLIC_EXPONENT, strlen(RSA_DEFAULT_PUBLIC_EXPONENT));
...
void test_genkey_valid_size_not_power_of_two(void) {
    ...
    // The rsa_genkey function in rsa.c only checks these min/max bounds, not for power-of-two.
    // Therefore, this test expects success.
    ...
    if (bitlen_to_test < RSA_MIN_KEY_BITLEN || bitlen_to_test > RSA_MAX_KEY_BITLEN) {
        ...
        rsa_error_t ret = rsa_genkey(&key, bitlen_to_test, RSA_DEFAULT_PUBLIC_EXPONENT, strlen(RSA_DEFAULT_PUBLIC_EXPONENT));
        ...
        // 5000 is within the valid range. Expect success.
        rsa_error_t ret = rsa_genkey(&key, bitlen_to_test, RSA_DEFAULT_PUBLIC_EXPONENT, strlen(RSA_DEFAULT_PUBLIC_EXPONENT));
        char msg[150];
        sprintf(msg, "Key generation with %d bits failed unexpectedly. rsa.c's rsa_genkey should allow this size.", bitlen_to_test);
        TEST_ASSERT_EQUAL_INT_MESSAGE(RSA_SUCCESS, ret, msg);

        if (ret == RSA_SUCCESS) {
            sprintf(msg, "Modulus N is NULL after genkey (%d bits)", bitlen_to_test);
            TEST_ASSERT_NOT_NULL_MESSAGE(key.n, msg);
            sprintf(msg, "Modulus N is zero after genkey (%d bits)", bitlen_to_test);
            ...
            TEST_ASSERT_MESSAGE(mpz_cmp_ui(key.e, 0) != 0, "Exponent E is zero");
            TEST_ASSERT_NOT_NULL_MESSAGE(key.d, "Private exponent D is NULL");
            TEST_ASSERT_MESSAGE(mpz_cmp_ui(key.d, 0) != 0, "Private exponent D is zero");
            TEST_ASSERT_NOT_NULL_MESSAGE(key.p, "Prime P is NULL");
            TEST_ASSERT_MESSAGE(mpz_cmp_ui(key.p, 0) != 0, "Prime P is zero");
            TEST_ASSERT_NOT_NULL_MESSAGE(key.q, "Prime Q is NULL");
            TEST_ASSERT_MESSAGE(mpz_cmp_ui(key.q, 0) != 0, "Prime Q is zero");
        }
    }
    rsa_free(&key);
}
